...
class Stnu:
    ...
    def verify_constraints(self):
        ...
        for edge in self.uncontrollable_edges:
            ...
            assert 0 <= edge.lower_bound <= edge.upper_bound            
            pairs.append((edge.fro, edge.to))
...
class NamedStnu(Stnu):
    ...
    def read_from_stdin(self):
        ...
        def read_edges(how_many, where_to):
            for _ in range(how_many):
                start_name, end_name, lower_bound, upper_bound = input().split(' ')
                ...
                where_to.append(StnuEdge(self._renaming[start_name],
                                          ...
                                          lower_bound,
                                          upper_bound))
        num_controllable = int(input())
        read_edges(num_controllable, self.controllable_edges)
        num_uncontrollable = int(input())
    ...
    def read_from_text(self, text):
        ...
        num_controllable = int(lines[0].strip())
        controllable_edges = lines[1:num_controllable + 1]

        num_uncontrollable = int(lines[num_controllable + 1].strip())
        uncontrollable_edges = lines[num_controllable + 2:num_controllable + num_uncontrollable + 2]

        def read_edges(edges, where_to):
            for edge in edges:
                start_name, end_name, lower_bound, upper_bound = edge.split(' ')
                lower_bound = float(lower_bound)
                upper_bound = float(upper_bound)
                for name in [start_name, end_name]:
                    if name not in self._renaming:
        ...
        read_edges(controllable_edges, self.controllable_edges)
        read_edges(uncontrollable_edges, self.uncontrollable_edges)
